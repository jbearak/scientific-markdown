...
- **Confluence-style panels** use a `~~~panel type=X` fence (closed by a bare `~~~`) where `X` is one of `info`, `note`, `error`, `success`, or `warning`. The fenced body is parsed as markdown and may include nested formatting, lists, and code blocks. Panel type names are also accepted in blockquote-marker form (e.g. `> [!INFO]`), and either syntax renders with the same border/icon. The `callout-style` frontmatter field (`github` | `confluence`, default `github`) disambiguates the shared name `note`: under `github` it renders blue like GitHub's note; under `confluence` it renders purple like Confluence's note panel. The original syntax form (blockquote vs. panel fence) is preserved through DOCX round-trip via a custom property so documents authored as Confluence panels re-export as panel fences.
